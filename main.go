...
func main() {
	...
	if claudeConfig.Enabled {
		...
		trigger := claude.NewTrigger(claudeConfig, waClient.SendTextMessage, waClient.SendTextMessageWithID, waClient.RevokeMessage, waClient.EditMessage, waClient.SendReaction, waClient.MyJID, store.GetChatMessagesWithNames, trustedUserStore.IsTrusted, store, mediaStore, sessionMgr)
